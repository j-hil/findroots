...
def test_vectorization() -> None:
    ...
    # `func1` has one root on (-4, 1) which intersects with each of these intervals
    intervals = (
        (1.1209, -12.3633),
        (-11.0574, 3.4692),
        (0.8966, -6.6580),
...
def test_distinct_roots() -> None:
    # `func2` has root 0.5 & 4 expect both to found
    # TODO: test failing - due to poor consideration of == case.
    tolerance = 1e-5
    roots, _ = findroot(func2, on=((0, 1), (3, 5)), tol=tolerance)

    assert_allclose(roots, (0.5, 4), atol=tolerance)
